...
export class TokenService extends BaseServiceAbstract<Token> {
  ...
  async newToken(data: Partial<User>): Promise<Token> {
    ...
    try {
      ...
      throw new HttpException(`Error create access token: ${error}`, HttpStatus.INTERNAL_SERVER_ERROR)
    }
  }

  async revokeToken(token: string, userId: string): Promise<boolean> {
    try {
      const foundToken = await this.tokenRepository.update({ access_token: token, userId }, { status: Status.INVALID })

      return !!foundToken
    } catch (error) {
      throw new HttpException(`Error revoke access token: ${error}`, HttpStatus.INTERNAL_SERVER_ERROR)
    }
  }
}
